...
- **Tools** — co se v aktivní session použilo: **Skills**, **Commands** (slash příkazy), **Built-in** (WebSearch/WebFetch/Task) a **MCP** servery + jejich tooly. Z `tool_use` bloků a `<command-name>` v transcriptu.
...
| Tools | aktivní `.jsonl` → `Skill` / `mcp__*` / `WebSearch…` tool_use + `<command-name>` (slash) |
| Usage | aktivní `.jsonl` → `usage` bloky assistant zpráv (context window, tokeny) |
| Instructions | `~/.claude/CLAUDE.md`, `/etc/claude-code/CLAUDE.md`, `<repo>/CLAUDE.md`, `<repo>/CLAUDE.local.md`, `…/memory/MEMORY.md` (+ fact soubory) |
...
```bash
code --install-extension claude-tools-0.1.0.vsix   # na WSL spusť z WSL terminálu
```

Pak **Developer: Reload Window**. Ikona Claude Tools naběhne v panelu.

### Build z kódu (Node 20+)
